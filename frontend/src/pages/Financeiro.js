...
function ListaFaturas({ onVerAluno }) {
  ...
  const ordenadas = [...filtradas].sort((a, b) => {
    ...
    // Dentro do mesmo status, desempata por vencimento (decrescente) e
    // depois por nome do aluno (crescente) — independe da direção da
    // coluna Status.
    if (cmp === 0 && sortCol === 'status') cmp = (b.data_vencimento || '').localeCompare(a.data_vencimento || '');
